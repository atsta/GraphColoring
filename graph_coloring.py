...
for _ in range(max_iterations):
    remaining = g.filterVertices("color == -1")
    aggregates = remaining.aggregateMessages(F.collect_set(AM.msg).alias("agg"), sendToDst=AM.src["id"])
    res = aggregates.withColumn("newColor", get_max_id_udf("id", "agg", lit(superstep))).drop("agg")
    
    new_vertices = g.vertices.join(res, on="id", how="left_outer").withColumnRenamed("color", "oldColor").withColumn("color", get_max_udf(F.col("oldColor"), F.col("newColor"))).drop("oldColor").drop("newColor")
